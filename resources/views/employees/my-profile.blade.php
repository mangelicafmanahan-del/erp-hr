...
@section('content')
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
                    ...
                    <span class="text-xs px-2 py-1 rounded-full {{ $statusColors[$employee->current_work_status] ?? 'bg-gray-100 text-gray-700' }}">
                ...
                <p class="text-xs text-gray-400 mt-1">
                    This is a read-only view. Contact HR to update any of this information.
                </p>

                <form
                    action="{{ route('my.profile.photo') }}"
                    method="POST"
    ...
    {{-- Personal Summary and Employment History --}}
        ...
        <div class="bg-white border rounded-lg p-6">
            ...
            <dl class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <dd>
                        {{ optional($employee->date_of_birth)->format('M d, Y') ?? '—' }}
                    </dd>
                </div>

                <div class="flex justify-between">
                    <dt class="text-gray-500">Contract Type</dt>
                    <dd>{{ $employee->contract_type ?? '—' }}</dd>
                </div>

                <div class="flex justify-between">
                    <dt class="text-gray-500">Hire Date</dt>
                    <dd>
                        {{ optional($employee->hire_date)->format('M d, Y') ?? '—' }}
                    </dd>
                </div>
            </dl>
        </div>
                ...
                <div class="flex justify-between items-start py-3 {{ !$loop->last ? 'border-b' : '' }}">
                    <div>
                        ...
                        <div class="text-sm text-gray-500">
                            {{ $history->company_name ?? 'This company' }}
                        </div>
    ...
    <div class="bg-white border rounded-lg p-6 mt-6">
        ...
        <form
            action="{{ route('my.documents.store') }}"
            method="POST"
            enctype="multipart/form-data"
    ...
    <div class="bg-white border rounded-lg p-6 mt-6">
        ...
        <p class="text-sm text-gray-500 mb-4">
            Choose how you would like to receive future payroll payouts.
            Check is the default method.
        </p>

        <form
            action="{{ route('my.profile.payment-preferences') }}"
            method="POST"
        ...
        >
            ...
            <div>
                ...
                <select
                    ...
                    id="payment_method"
                    required
                    class="border rounded-md px-3 py-2 text-sm w-full sm:w-80"
                    onchange="toggleBankFields()"
                >
                    <option
                        value="Check"
                        @selected(($employee->payment_method ?? 'Check') === 'Check')
                    >
                        Check
                    </option>
                    ...
                    <option
                        value="Bank Transfer"
                        @selected(($employee->payment_method ?? 'Check') === 'Bank Transfer')
                    >
                        Bank Transfer
                    </option>
                    ...
                    <option
                        value="Cash"
                        @selected(($employee->payment_method ?? 'Check') === 'Cash')
                    >
                        Cash
                    </option>
                </select>
            ...
            <div
                id="bank-fields"
                class="grid grid-cols-1 sm:grid-cols-3 gap-3 {{ ($employee->payment_method ?? 'Check') === 'Bank Transfer' ? '' : 'hidden' }}"
            >
                <div>
                    <label class="text-xs text-gray-500 block mb-1">
                        Bank Name
